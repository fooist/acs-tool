...
curl https://api.census.gov/data/$1/acs/acs$2/variables.json | \
  jq -r ".variables| to_entries | .[] | [.key, .value.label, .value.concept, .value.predicateType]|join(\"|\")" | \
...
curl https://api.census.gov/data/$1/acs/acs$2/groups.json | \
  jq -r ".groups[]|[.name, .description, .variables]|join(\"|\")" >  data/raw/acs/$1_$2_yr/metadata/tables.csv
